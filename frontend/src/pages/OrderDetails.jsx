...
export default function OrderDetails() {
  ...
  if (err) return <div className="page"><div className="error">{err}</div><Link className="btn-outline" to="/my-orders">Back to My Orders</Link></div>;
  ...
  return (
    <div className="page narrow">
      ...
      <div className="order-detail-card">
        ...
        <div className="track-current">Current status: <span className={`status ${order.status}`}>{statusLabel(order.status)}</span></div>
      </div>
      ...
      <div className="order-detail-grid">
        ...
        <div className="order-detail-card">
          ...
          <p className="muted">{address.name}<br />{address.phone}<br />{address.address}, {address.city} {address.postalCode || ''}</p>
          <h3>Payment</h3>
          <p className="muted">Method: {order.payment_method}<br />Status: {order.payment_status}</p>
        </div>
      </div>
      ...
      <div className="order-detail-card summary-block">
        ...
        {Number(order.discount) > 0 && <div className="summary-line"><span>Discount</span><b>-{formatPrice(order.discount)}</b></div>}
        <hr />
