...
export default function MyOrders() {
  ...
  useEffect(() => {
    if (user) myOrders().then(setRows).finally(() => setLoading(false));
  ...
  return (
    <div className="page">
      ...
      <div className="order-list">
        {rows.map((o) => (
          <div className="order-card-full" key={o._id}>
            <div className="order-card-head">
              <div>
                <b>{o.order_number}</b>
                <small>{new Date(o.createdAt || o.created_at).toLocaleString()}</small>
              </div>
            ...
            <div className="order-card-actions">
              <Link className="btn-outline" to={`/my-orders/${o._id}`}>View Details</Link>
              <Link className="btn-primary" to={`/my-orders/${o._id}`}>Track Order</Link>
            </div>
          </div>
        ))}
        {!rows.length && (
